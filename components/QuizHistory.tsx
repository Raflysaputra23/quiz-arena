...
const QuizHistory = memo(({ search }: { search: string }) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (error) {
        toastError("Gagal memuat riwayat");
        startTransition(() => setLoading(false));
        return;
      }

      const mapped: QuizHistory[] = (data ?? []).map((q: any) => ({
        id: q.id,
        title: q.title,
  ...
  const getModeLabel = (mode: string) => {
    switch (mode) {
      case "speed":
        return { label: "⚡ Speed", color: "bg-yellow-500/20 text-yellow-500 border-yellow-500/50" };
      case "survival":
        return { label: "💀 Survival", color: "bg-purple-500/20 text-purple-500 border-purple-500/50" };
      case "battle":
        return { label: "⚔️ Battle", color: "bg-red-500/20 text-red-500 border-red-500/50" };
      default:
        return { label: "✨ Normal", color: "bg-primary/20 text-primary border-primary/50" };
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <span className="px-2 py-0.5 rounded-full text-xs bg-secondary text-muted-foreground animate-pulse">Menunggu</span>
        );
    }
  };
  ...
  return (
    <>
      <div className="space-y-4" id="riwayat-quiz">
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 glass rounded-2xl shadow">
            {!search ? (
              <>
              ...
              <>
                ...
                <h2 className="font-poppins text-md text-muted-foreground">Tidak ada judul yang cocok dengan pencarian</h2>
              </>
            )}
          </motion.div>
        ) : (
          <AnimatePresence>
            {filtered.map((quiz, i) => (
              <motion.div
                ...
                transition={{ delay: i * 0.05, ease: "easeIn" }}
              ...
              >
                <div className="p-5 flex items-start lg:items-center lg:justify-between lg:flex-row overflow-hidden flex-col gap-4">
                  ...
                  <div className="flex gap-2 shrink-0 flex-wrap">
                    <Button size="sm" variant={"primary"} onClick={() => setShowModalPower(quiz.id)} title="Main Lagi">
                          ...
                          >
                            <div className="flex items-center justify-between gap-2 flex-wrap">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                  className={`shrink-0 text-xs px-2 py-0.5 rounded-full ${getModeLabel(session.mode).color}`}
                                >
                            ...
                            {session.participants.length > 0 && (
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {session.participant_count > 5 && (
                                  <span className="text-xs text-muted-foreground">+{session.participant_count - 5} lainnya</span>
                                )}
                              </div>
                            )}
                          </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-end gap-1">
                ...
                <Button size="sm" className="cursor-pointer" variant="primary" onClick={() => setSelectedId(null)}>
                  Batal
                </Button>
              </div>
            </motion.div>
            ...
            >
              ...
              <FieldGroup className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                <FieldLabel
                  ...
                  className={
                    "p-4 bg-gold/10 w-full text-gold border-gold rounded-xl flex gap-4 border transition has-data-[state=checked]:border-gold has-data-[state=checked]:bg-gold/20 dark:has-data-[state=checked]:bg-gold/20"
                  }
                >
                  <Checkbox checked={selected.doublePoints} className="text-gold border-gold peer" />
                  <FieldContent>
                    <FieldTitle>
                      <Star className="w-4 h-4" /> 2x Double Points
                    </FieldTitle>
                    <FieldDescription>Dapatkan 2x poin untuk jawaban benar</FieldDescription>
                  </FieldContent>
                </FieldLabel>
                <FieldLabel
                  onClick={() =>
                ...
                >
                  ...
                  <FieldContent>
                    <FieldTitle>
                      <Shield className="w-4 h-4" /> Fifty Fifty
                    </FieldTitle>
                    <FieldDescription>Hapus 2 pilihan ganda jawaban salah</FieldDescription>
                  </FieldContent>
                </FieldLabel>
                <FieldLabel
                  onClick={() =>
                ...
                >
                  ...
                  <FieldContent>
                    <FieldTitle>
                      <Zap className="w-4 h-4" /> Efek Lightning
                    </FieldTitle>
                    <FieldDescription>Menghilangkan pertanyaan selama 4 detik</FieldDescription>
                  </FieldContent>
                </FieldLabel>
                <FieldLabel
                  onClick={() =>
                ...
                >
                  ...
                  <FieldContent>
                    <FieldTitle>
                      <Snowflake className="w-4 h-4" /> Efek Freeze
                    </FieldTitle>
                    <FieldDescription>Tidak dapat menjawab selama 4 detik</FieldDescription>
                  </FieldContent>
                </FieldLabel>
              </FieldGroup>
              <div className="flex items-center justify-end gap-1">
                <Button variant="destructive" size="sm" className="cursor-pointer" onClick={() => setShowModalPower(null)}>
                  Batal
                </Button>
                <Button
                  size="sm"
...
QuizHistory.displayName = "QuizHistory";
